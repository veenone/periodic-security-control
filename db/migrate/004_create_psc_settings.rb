...
class CreatePscSettings < ActiveRecord::Migration[5.2]
  def change
    unless table_exists?(:psc_settings)
      create_table :psc_settings do |t|
        t.references :project, type: :integer, null: false, foreign_key: true, index: { unique: true }
        t.references :default_tracker, type: :integer, foreign_key: { to_table: :trackers }
        t.references :default_priority, type: :integer, foreign_key: { to_table: :enumerations }
